...
export async function writeAudit(entry: AuditEntry) {
    ...
    try {
        if (AppDataSource && (AppDataSource as any).isInitialized) {
            ...
            const params = [entry.actor_id || null, entry.actor_type || null, entry.action, entry.resource || null, entry.resource_id || null, JSON.stringify(entry.data || {}), entry.outcome || null];
