...
- Steps:
  1. Add CI workflow(s) to run: `pnpm build`, `pnpm preview` (port 4173), `vitest` unit tests, and Playwright E2E tests. Ensure ephemeral DB or test DB used for migration tests (Issue #29).
  ...
  4. Add accessibility checks (Playwright Axe or pa11y) into CI for critical flows (Issue #33).
...
- Steps:
  1. Add Playwright accessibility checks (axe) into CI and create issue-tracking for outstanding failures (Issue #33).
...
- Add CI jobs for build + vitest + Playwright and a migration test (#29, #30).
...
*Plan created: 2025-11-27T00:00Z*
